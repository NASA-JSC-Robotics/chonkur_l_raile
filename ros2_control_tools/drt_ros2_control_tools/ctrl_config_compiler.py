...
def compile_controller_configurations(config_paths_file: str, output_path: str, debug=False) -> bool:
    ...
    config_paths_file -- String filepath to yaml containing packages and paths to
    control configuration files in the form of
                         {<pkg>:
                             path: <str path to control config file>}
    output_path -- Output compiled controller configuration filepath including desired filename
    """
    ...
    if not pathlib.Path(config_paths_file).is_file():
        print(
            bcolors.HEADER
            + "(compile_controller_configurations) "
            + bcolors.ENDC
            + bcolors.FAIL
            + bcolors.BOLD
            + bcolors.UNDERLINE
            ...
            + bcolors.ENDC
            + bcolors.FAIL
            + f" file at {config_paths_file} not found, exiting..."
            + bcolors.ENDC
        )
        error_status = True
        return error_status

    with open(config_paths_file) as file:
    ...
    for config in configuration_paths:
        ...
        if not configpath.is_file():
            print(
                ...
                + bcolors.ENDC
                + bcolors.WARNING
                + f" failed to find file at {configpath}, skipping..."
                + bcolors.ENDC
            )
            continue
        if configpath.suffix != ".yaml":
            print(
                ...
                + bcolors.ENDC
                + bcolors.WARNING
                + f" file at {configpath} is not a controller configuration file, skipping..."
                + bcolors.ENDC
            )
            continue
        else:
        ...
        if ctrlr_name_count[name] > 1:
            if name != "joint_state_broadcaster":
                print(
                    ...
                    + bcolors.ENDC
                    + bcolors.FAIL
                    + f" Duplicate controller {name} found, exiting..."
                    + bcolors.ENDC
                )
                error_status = True
                return error_status
                ...
                if debug:
                    print(
                        ...
                        + bcolors.ENDC
                        + bcolors.OKCYAN
                        + "Removing duplicated joint_state_broadcaster controllers"
                        + bcolors.ENDC
                    )
    ...
    for config in configs:
        for ctrlr_name in ctrlr_names:
            ...
            if ctrlr_name in configs[config]["controller_manager"]["ros__parameters"]:
                compiled_cfg["controller_manager"]["ros__parameters"][ctrlr_name] = configs[config][
                    "controller_manager"
                ]["ros__parameters"][
                    ctrlr_name
                ]  # Compile controller types
