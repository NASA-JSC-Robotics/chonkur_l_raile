...
class ControllerCancelServiceExtractor:
    def __init__(self, include_gripper: bool, node: Node):
        """The constructor for the ControllerCancelServiceExtractor Class,
        which extracts the types of controllers to be cancelled.
        Args:
            include_gripper (bool): whether or not to include the gripper in the list of controllers to cancel.
        """
        # key: standard controller types for robot, value: the standard request name for the cancel
        # service of that controller type (not fully rectified name)
    ...
    def add_cancel_service(self, list_ctrlrs_resp: list[ListControllers_Request]):
        ...
        self.node.get_logger().debug(
            f"{self.node.get_name()}: this is the list of all controllers to be cancelled {self.cancel_service_list}"
        )
...
class EStopSafety(Node):
    ...
    def __init__(
        self,
        estop_topic: str,
        estop_msg_type: EStopROSMsg,
        stop_gripper: bool = True,
        list_controllers_service_name: str = "/controller_manager/list_controllers",
    ):
        ...
        Args:
            ...
            list_controllers_service_name (str, optional): the name of the service where it can retrieve the list
            the controllers. Defaults to '/controller_manager/list_controllers'.
        """
        ...
        self.failed_cancel_timer = self.create_timer(
            timer_period, self.failed_cancel_callback, callback_group=self.failed_cancel_cb_group
        )
        ...
        self.estop_subscriber = self.create_subscription(
            ...
            # qos will keep the last 10 messages (this is the depth of the messaging history).
            # Needs to match the qos_profile of the publisher to work
            qos_profile=10,
            callback_group=self.estop_cb_group,
        )

    def estop_triggered(self, msg: EStopROSMsg) -> bool:
        """Evaluate the estop ros msg to determine if the estop has been triggered.
        Returns true once the estop has been triggered and false all other times.
        To be overridden by the robot specific method.
    ...
    def estop_listener_callback(self, msg: EStopROSMsg):
        ...
        """
        self.get_logger().debug(f"{self.get_name()} Estop msg: {msg}")
        if self.estop_triggered(msg):
            ...
            self.get_logger().info(
                f"{self.get_name()} cancelled all {self.extractor.controller_dict.keys()} controllers"
            )
    ...
    def get_cancel_service_names(self) -> bool:
        ...
        self.list_ctrlrs_client = self.create_client(
            ListControllers, self.list_controllers_service_name, callback_group=self.controllers_cb_group
        )
        ...
        if not self.list_ctrlrs_client.wait_for_service(timeout_sec=2.0):
            ...
            self.get_logger().fatal(
                f"{self.get_name()}: unable to reach {self.list_ctrlrs_client.srv_name}. \
                to get the list of active controllers that need to be cancelled. \
                Active controllers were not stopped, so there may be unexpected movement. \
                Be careful when re-enabling the robot."
            )
            return False
            ...
            while not cancel_client.wait_for_service(timeout_sec=1.0) and not failed_find_service:
                ...
                if timeout_counter >= 2:
                    ...
                    self.get_logger().error(
                        f"unable to reach {client_name} to cancel its trajectory, so there may be \
                        unexpected movement. Be careful when re-enabling the robot."
                    )
    ...
    def failed_cancel_callback(self):
        for client_name in self.failed_cancel_list:
            self.get_logger().error(
                f"unable to reach {client_name} to cancel its trajectory, so there may be unexpected movement. \
                Be careful when re-enabling the robot."
            )
